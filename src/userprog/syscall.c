...
/* TODO: Functions that check the if the arguments are in
   valid user memory. */
static bool is_valid_uaddr(const void* vaddr) { return true; }

static bool is_valid_user_memory(const void* vaddr, size_t size) { return true; }

static bool are_valid_args(const int32_t* args, size_t num_args) { return true; }

static bool is_valid_string(const char* str) { return true; }
...
static void syscall_handler(struct intr_frame* f) {
  ...
  if (!are_valid_args(args, 1) || args[0] >= 32 ||
      !are_valid_args(args + 1, map[args[0]].num_args)) {
    // Kill process
  }
